...
var TaroCreateInf = function (type, taro_file_path, option) {
  ...
  let newF = taro_file_path.replace(option.file, interFace_Name.trim() + ".java");
  console.log(newF);
...
TaroCreateInf.createInf = function (arr) {
  ...
  for (let i = 0; i < interfaces.length; i++) {
    let interfaceObj = interfaces[i];
    nodejsCode += `app.${interfaceObj.returnType.toLowerCase()}('${interfaceObj.methodName}', function(req, res) {\n\t// Implementation\n});\n`;
  }
